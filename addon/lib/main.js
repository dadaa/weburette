...
function lightOn(pin, time) {
    arduino.analogWrite(pin.number, 255);
    pin.timerid = timers.setTimeout(lightOff, 0, pin, time);
}

function lightOff(pin, time) {
    arduino.analogWrite(pin.number, 0);
    if (4 <= pin.number && pin.number <= 8) {
        pin.timerid = -1;
    } else {
        timers.setTimeout(timerOff, time, pin);
    }
}

function timerOff(pin) {
    pin.timerid = -1;
}
